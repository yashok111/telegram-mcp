...
2. **Mention-based** — tokens of the form `@<word>` (grammar `[A-Za-z0-9_-]+`, case-insensitive) route to one or more shims. Resolution order inside the mention step:
   ...
   - Aliases are **sticky**: an `@sN` is bound to a project's reuse-key (`workdir:`/`label:`) and persisted in `access.json → alias_by_key`, so a session keeps the same alias across reconnects and daemon restarts; an offline project's number is reserved so a new project can't take it. Wired via `Router.SetStickyAliasStore` at boot; nil store (tests) falls back to legacy lowest-free allocation.
...
**Topic headers (`internal/daemon/header.go`):** in forum mode every topic gets a pinned header message the daemon maintains — sent + `pinChatMessage`'d on topic creation (`HeaderManager.Ensure`, called from the hello handler after `BindTopic`), edited in place as the owning shim's state changes. Render (plain text, **not** MarkdownV2 — workdir/label are `/._-`-heavy so escaping buys nothing and would perturb the dedup hash):
...
State icons: 🟢 idle · 🟡 busy · 🔵 awaiting permission · ⚪ disconnected · 🔴 closed. **Busy-state is daemon-inferred, not shim-reported** — CC does not push tool_start/tool_end to MCP servers (only `permission_request` reaches the daemon), so there is NO IPC/MCP change. Hooks: `Notifier.DeliverInbound`→🟡, `Handlers.HandleBroadcastPermission`→🔵 (carries the tool name from `permDetails` — the ONLY state with a tool name; plain busy has none), `Notifier.ResolvePermission`→🟡 (agent resumes after approval), shim outbound `HandleSendMessage`/`HandleSendFile`→🟢 (**not** `HandleEditMessage` — edits are interim progress, flipping to idle there would flicker 🟢 mid-work), `OnDisconnect`→⚪ (via `daemon.headerDisconnect`, which must run before `Router.Drop` so `TopicForShim` still resolves), `TopicCloser.CloseTopic`→🔴 (after the topic closes; the bot edits the pinned message as topic admin). `cc_pid` is intentionally omitted from the render (never transmitted daemon-side). The `headerState`/`setShimHeaderState` helpers resolve shim→topic via `Router.TopicForShim` so `TopicID` reads stay behind `r.mu`; `flush` pulls identity (`Router.HeaderIdentity`) *outside* `m.mu` to avoid a lock-order inversion with `Router.SetLabel`→`headerHook` (`m.Refresh`, which briefly takes `m.mu` to mark dirty). Persisted in `TopicMeta.{HeaderMessageID,HeaderPinned,HeaderRenderHash}` so a daemon restart re-binds the existing pinned message instead of resending. Edits are coalesced + per-topic rate-limited (`HeaderManager.Run`'s refresh ticker) with a per-topic `flushing` guard serializing IO; an FNV-64a content hash skips no-op edits (and Telegram's "message is not modified" 400). Recovery: `editMessageText` "message to edit not found" → recreate + re-pin (new id persisted); pin 400 (no `can_pin_messages`) → graceful degrade (`HeaderPinned=false`, header still visible, pin retried whenever a later flush sees `false`). Env (read by `cmd/server.buildHeaderManager`): `TELEGRAM_TOPIC_HEADER=1|0` (feature flag, default on; `0|false|no|off`), `TELEGRAM_TOPIC_HEADER_REFRESH=5s` (min edit interval / debounce window), `TELEGRAM_TOPIC_HEADER_TICK=60s` (background uptime/idle-for re-render cadence). `buildHeaderManager` returns nil when forum mode is off or the flag is off; `Daemon.Header==nil` disables the worker and every hook is nil-safe. **Known limits** (accepted, not bugs): busy has no tool name (daemon can't see the running tool); a shim that finishes without sending stays 🟡 until the next inbound/disconnect (the "last activity: Ns ago" line signals staleness); an externally-*unpinned* (not deleted) header is not re-pinned while `HeaderPinned=true` — cosmetic, `getChat` polling not worth it; editing the header in a closed forum topic relies on the bot's topic-admin rights (verified Telegram behavior — a failure logs a warn, no crash; worth a live smoke test).

**Topic naming + lifecycle:**
- **Title = project identity** (`label` or workdir basename), NOT the alias — `buildTopicName` (`forum.go`). The alias reshuffles across restarts, so embedding it in the persistent title made the list lie about "who is who"; the live `@sN` rides the pinned header instead (which is always bound to the current owner). Legacy `@sN — x` titles self-migrate to project-only on the next reuse via `resyncName`. Degenerate workdir (`/`, empty) falls back to the raw path or the literal `session` — never the alias, so the title never churns.
- **Orphan sweep** (`internal/daemon/orphan_sweep.go`) — a topic whose owner disconnected (`LockedBy==""`, `ReleasedAt>0`) longer than `TELEGRAM_TOPIC_ORPHAN_AFTER` (default 7d, `=0` disables) is closed via `TopicCloser`, then deleted by `TopicSweep` after `TELEGRAM_TOPIC_PURGE_AFTER` (default 14d). The close is race-safe: an atomic `claim` re-checks `LockedBy` and strips the reuse key in one `store.Mutate`, so a shim reattaching in the same instant either wins the lock (claim bails) or gets a fresh topic (its reuse key is gone) — a live shim is never closed out. `ReleasedAt` is stamped in `Forum.ReleaseLock`, cleared on every re-lock. `TOPIC_ID_INVALID` is classified permanent (`bot/apierr.go`) so both sweeps drop a gone topic instead of retrying forever.
- **Disconnected/closed headers freeze** — `HeaderManager.markActiveDirty` skips ⚪ disconnected and 🔴 closed topics, so the 60s uptime tick doesn't churn an `editMessageText` per interval on topics with no live owner (the relative-time line would otherwise change every tick and defeat the content-hash dedup).
...
**Per-chat effort (`/effort`):** DM `/effort <low|medium|high|xhigh|max>|show|clear` sets the model + `MAX_THINKING_TOKENS` budget for *future* `/spawn` and `/bg` invocations from that chat. Mapping (`internal/bot/effort.go:effortConfigs`): `low → claude-haiku-4-5 + 0`, `medium → claude-sonnet-4-6 + 8000`, `high → claude-opus-4-8 + 16000`, `xhigh → claude-opus-4-8 + 32000`, `max → claude-opus-4-8 + 64000`. Persisted as `access.State.EffortByChat map[chatID]levelName` so survives daemon restarts; level string (not resolved model) is stored so future mapping changes don't invalidate disk state. `handleSpawnCommand`/`handleBgCommand` look up `EffortByChat[chatID]`, call `ResolveEffort`, and fill `SpawnRequest.Model` + `ThinkingTokens` (zero values = "use daemon defaults"). Daemon-side `SpawnRunner.Spawn` and `BgRunner.runTask` translate non-zero fields into `--model=<id>` argv + `MAX_THINKING_TOKENS=<N>` env on the spawned `claude` process; both `filterEnv` pre-existing `MAX_THINKING_TOKENS=` from the inherited environment before appending so the request-supplied value wins (no duplicate entries). `--model=` is appended to `cfg.ClaudeArgs` via `slices.Clone` so concurrent `/spawn` calls don't race on the shared base slice. Unknown level strings stored on disk (e.g., from manual `access.json` editing) fall back to defaults — `ResolveEffort` returns `ok=false`, the dispatcher leaves Model/ThinkingTokens at zero. `/status` renders the current per-chat effort via `renderEffortLine`; `/help` and `SetMyCommands` advertise the command. Only DM senders in `access.json.AllowFrom` reach `handleEffortCommand` (same gate as the rest of the command surface). Existing shims keep their boot-time model until respawn; the persisted change only applies to *new* `/spawn` and `/bg` clients.
...
The agent should know its own shim alias from turn 1 so `@s2 do X` mentions work without needing inbound message metadata. Correlation is **version-gated**. The shim files its snapshot by **CC's pid** (`os.Getppid()`) and tags it with `cc_session_id`. CC ≤2.1.143 did not expose its session id to MCP/plugin processes (neither via `initialize` nor env, confirmed empirically), so `self` walks the PPID chain to the `<cc_pid>.json` filename. CC **2.1.154+** sets `CLAUDECODE=1` + `CLAUDE_CODE_SESSION_ID` in MCP-subprocess and hook env (confirmed live 2026-05-28), so `self` now prefers an exact `cc_session_id` match — robust against PID recycling and unusual process trees — and falls back to the PPID walk.
...
1. **Shim side** — on `Wire()` success, the shim writes a per-session snapshot to:
   ...
   `{alias, shim_id, shim_id_prefix, cc_pid, shim_pid, cc_session_id?, workdir, label?, started_at, mode}`. `cc_session_id` comes from `CLAUDE_CODE_SESSION_ID` in env. On CC ≤2.1.153 that var was set for Bash + hooks but **not** MCP servers, so the shim's copy was empty and non-load-bearing; CC 2.1.154+ sets it for MCP subprocesses too, so it now populates and `self` uses it as the primary correlation key (see point 2).

2. **CC side** — `telegram-mcp self` locates the snapshot two ways (`cmd/server/self.go`): when CC exposes a trusted session id (2.1.154+; `CLAUDECODE=1` gates trusting `CLAUDE_CODE_SESSION_ID`) it scans `sessions/*.json` for a matching `cc_session_id`, otherwise it walks the PPID chain (up to 8 hops) for the first ancestor whose `/proc/<pid>/comm` starts with `claude` and reads `<cc_pid>.json`. Override the walk by exporting `CC_PID=<pid>`. Wire it as a SessionStart hook in `~/.claude/settings.json`:
