...
**Daemon-spawned CC clients (`/spawn`):** DM `/spawn [--in <dir>]` forks a fresh `claude --dangerously-load-development-channels plugin:telegram@local-yakov` via `pty.New()` + `pty.CommandContext` (github.com/aymanbagabas/go-pty) in the resolved workdir. The spawned CC loads the telegram plugin → plugin starts its own shim subprocess → shim does `hello` over IPC → `Router.Register` allocates a fresh `@sN` alias. From that point the spawn is a **regular shim** — addressable via `@<alias>`, `reply_to`, `/sessions`, `/use`, `/label`; no special routing path on the bot side. The pty exists only because real `claude` refuses to start without a TTY; its output is drained to `io.Discard` (TUI rendering goes nowhere — all real traffic flows through the shim's MCP/IPC). Linkage between spawn-record and shim is via the `TELEGRAM_SPAWN_ID` env var: the daemon stamps it before `cmd.Start()`, the shim reads it from env and includes it in the `hello` params, the daemon stores it on `Shim.SpawnID`, and `/spawn list` cross-references `Router.Snapshot()` to print `<spawn_id> · running · @sN · pid=… · <ago> · <workdir>`. Subcommands: `/spawn list`, `/spawn cancel <id>` (SIGTERM → 5s wait → SIGKILL → close pty; `PR_SET_PDEATHSIG` in the spawned shim handles its own teardown chain, after which `Router.Drop` fires via the IPC `OnDisconnect` hook). No idle timer (Router already tracks LastOutbound), no chat-pin, no stdin forwarding — `SpawnRunner` only owns subprocess lifecycle. Implementation in `internal/daemon/spawn.go`; per-call dispatch in `internal/bot/spawn.go:handleSpawnCommand`. Env vars: `TELEGRAM_SPAWN_MAX_PARALLEL` (default 3), `TELEGRAM_SPAWN_HARD_TIMEOUT` (default 24h — absolute cap), `TELEGRAM_SPAWN_DEFAULT_WORKDIR` (else `$HOME`), `TELEGRAM_SPAWN_RATE_PER_HOUR` (per-user, default 5), `TELEGRAM_SPAWN_CLAUDE_BIN` (default `"claude"`), `TELEGRAM_SPAWN_CLAUDE_ARGS` (space-separated; default `--dangerously-load-development-channels plugin:telegram@local-yakov`). Tests in `internal/daemon/spawn_test.go` + `internal/bot/spawn_test.go`; integration test in `internal/daemon/spawn_integration_test.go` (build tag `integration`) drives a `/bin/sh` SIGTERM-trapping stub through real `pty.New()` to verify env propagation and Cancel teardown. Same threat model as `/bg` applies: workdir is operator-trusted via the allowlist; spawned `claude` inherits the daemon's UID. New IPC protocol field: `hello.spawn_id` (string, optional) — empty for user-launched shims, non-empty for daemon-spawned ones.
