...
| `/topics list`       | DM             | Render every known topic + lock state (DM-only, allowlisted). |
...
Every shim's activity is captured to a per-shim log sink under
`~/.claude/channels/telegram/shims/<shim_id>.log` (`TELEGRAM_SHIM_LOG_*`), so
you can inspect one session without grepping the combined `daemon.log`.
